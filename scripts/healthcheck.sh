#!/bin/bash
# /opt/healthcheck/check.sh - instalado pelo auto-apply.sh T16
# Cron */5min com flock. Alerta Telegram so com COOLDOWN (1 alerta/hora por hash).

set +e
LOG=/var/log/hairtech-healthcheck.log
exec >> "$LOG" 2>&1

TS=$(date -Iseconds)
TG_TOKEN="${TELEGRAM_BOT_TOKEN:-8470054351:AAEBUfBP1oTT2Yx9W5J5_sgFCfxoJeOeXEQ}"
TG_CHAT="${TELEGRAM_CHAT_ID:-8713631351}"
COOLDOWN_DIR=/tmp/hairtech-alert-cooldown
mkdir -p "$COOLDOWN_DIR"

# Alerta com cooldown por hash do conteudo
alert_cd() {
  local CATEGORIA="$1"
  local MSG="$2"
  local HORA_ATUAL=$(date +%Y%m%d%H)
  local MARK="$COOLDOWN_DIR/${CATEGORIA}-${HORA_ATUAL}"
  if [ -f "$MARK" ]; then
    echo "[$TS] $CATEGORIA: alerta SUPRIMIDO (ja alertou nesta hora)"
    return
  fi
  touch "$MARK"
  # Limpa marks antigos (>24h)
  find "$COOLDOWN_DIR" -mtime +1 -delete 2>/dev/null

  local TRIMMED="${MSG:0:3500}"
  curl -s -X POST "https://api.telegram.org/bot${TG_TOKEN}/sendMessage" \
    --data-urlencode "chat_id=${TG_CHAT}" \
    --data-urlencode "text=HairTech alert ${TS}
${TRIMMED}" > /dev/null 2>&1
  echo "[$TS] ALERT [$CATEGORIA]: $MSG"
}

# Check 1: Postgres
if ! docker exec hairtech-postgres pg_isready -U hairtech > /dev/null 2>&1; then
  alert_cd "postgres" "Postgres nao responde"
fi

# Check 2: AV (com self-heal AGRESSIVO se em restart loop)
if [ -f /home/user/nodejs/ALLOW_RESTART.flag ]; then
  AV_STATE=$(docker inspect assistente-virtual --format '{{.State.Status}}' 2>/dev/null)
  AV_RESTARTS=$(docker inspect assistente-virtual --format '{{.RestartCount}}' 2>/dev/null)
  if [ "$AV_STATE" != "running" ]; then
    LOG_SNIPPET=$(docker logs assistente-virtual --tail 50 2>&1 | tail -15)
    HEAL_MARK="$COOLDOWN_DIR/av-heal-$(date +%Y%m%d%H)"

    # Se em restart loop (state=restarting com muitos restarts), para o container
    if [ "$AV_STATE" = "restarting" ] && [ -n "$AV_RESTARTS" ] && [ "$AV_RESTARTS" -gt 5 ]; then
      if [ ! -f "$HEAL_MARK" ]; then
        touch "$HEAL_MARK"
        echo "[$TS] AV em restart loop ($AV_RESTARTS restarts) - parando pra evitar churn"
        docker stop assistente-virtual > /dev/null 2>&1
        alert_cd "av-loop" "AV em RESTART LOOP ($AV_RESTARTS restarts) - container PARADO ate proximo deploy. Logs: $LOG_SNIPPET"
      fi
    else
      alert_cd "av-state" "AV state=$AV_STATE (restarts=$AV_RESTARTS). Logs: $LOG_SNIPPET"
    fi
  fi
fi

# Check 3: Traefik
T_STATE=$(docker inspect traefik-traefik-1 --format '{{.State.Status}}' 2>/dev/null)
if [ "$T_STATE" != "running" ]; then
  alert_cd "traefik" "Traefik state=$T_STATE"
fi

# Check 4: OpenClaw com self-heal 1x/hora
OC_STATE=$(docker inspect hairtech-openclaw --format '{{.State.Status}}' 2>/dev/null)
if [ "$OC_STATE" != "running" ] && [ -n "$OC_STATE" ]; then
  HEAL_MARK="$COOLDOWN_DIR/oc-heal-$(date +%Y%m%d%H)"
  if [ ! -f "$HEAL_MARK" ]; then
    touch "$HEAL_MARK"
    docker start hairtech-openclaw > /dev/null 2>&1
    sleep 5
    OC_STATE=$(docker inspect hairtech-openclaw --format '{{.State.Status}}' 2>/dev/null)
    echo "[$TS] OpenClaw self-heal tentado, new_state=$OC_STATE"
  fi
  if [ "$OC_STATE" != "running" ]; then
    LOG_SNIPPET=$(docker logs hairtech-openclaw --tail 20 2>&1 | tail -6)
    alert_cd "openclaw" "OpenClaw state=$OC_STATE (self-heal falhou). Logs: $LOG_SNIPPET"
  fi
fi

# Check 5: WAHA ANA (preserva pareamento - SO docker start, NUNCA restart)
WA_STATE=$(docker inspect whatsapp-ana --format '{{.State.Status}}' 2>/dev/null)
if [ "$WA_STATE" != "running" ] && [ -n "$WA_STATE" ]; then
  HEAL_MARK="$COOLDOWN_DIR/wa-heal-$(date +%Y%m%d%H)"
  if [ ! -f "$HEAL_MARK" ] && [ "$WA_STATE" = "exited" ]; then
    touch "$HEAL_MARK"
    docker start whatsapp-ana > /dev/null 2>&1
    sleep 8
    WA_STATE=$(docker inspect whatsapp-ana --format '{{.State.Status}}' 2>/dev/null)
    echo "[$TS] WAHA ANA self-heal tentado, new_state=$WA_STATE"
  fi
  if [ "$WA_STATE" != "running" ]; then
    alert_cd "waha" "WAHA ANA state=$WA_STATE (pareamento pode ter perdido)"
  fi
fi

# Check 6: Webhook Meta publico (so se ALLOW_RESTART)
if [ -f /home/user/nodejs/ALLOW_RESTART.flag ]; then
  CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 6 \
    "https://hairtech.org/webhook?hub.mode=subscribe&hub.verify_token=hairtech_token_2026&hub.challenge=hc_$(date +%s)" 2>/dev/null)
  if [ "$CODE" != "200" ]; then
    alert_cd "webhook" "Webhook Meta retornou HTTP=$CODE"
  fi
fi

echo "[$TS] healthcheck OK"
